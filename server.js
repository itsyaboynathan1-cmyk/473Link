...
const server = http.createServer(async (req, res) => {
  ...
  // Reviews — GET (query ?businessId=xxx)
  if (pathname === '/api/reviews' && req.method === 'GET') {
    const businessId = reqUrl.searchParams.get('businessId');
    const runtime = getRuntime();
    const reviews = businessId
      ? runtime.reviews.filter((r) => r.businessId === businessId)
      : runtime.reviews;
    return sendJson(res, 200, { reviews: [...reviews].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)) });
  }
  ...
  if (pathname === '/api/reviews' && req.method === 'POST') {
    ...
    try {
      ...
      if (!businessId || !rating) return sendJson(res, 400, { error: 'businessId and rating (1-5) are required.' });
      const runtime = getRuntime();
      const existing = runtime.reviews.find((r) => r.businessId === businessId && r.userId === auth.user.id);
      if (existing) {
        existing.rating = rating;
        existing.text = text;
        existing.updatedAt = new Date().toISOString();
        saveRuntime(runtime);
        return sendJson(res, 200, { review: existing });
      }
      const review = {
        id: createId('REV'),
        businessId,
        userId: auth.user.id,
        authorName: String(auth.user.name || 'Anonymous'),
        rating,
        text,
        createdAt: new Date().toISOString(),
      ...
      return sendJson(res, 400, { error: error.message || 'Unable to submit review.' });
    }
  }
  ...
  if (pathname === '/api/admin/events' && req.method === 'POST') {
    ...
    try {
      ...
      return sendJson(res, 400, { error: error.message || 'Unable to create event.' });
    }
  }

  const adminEventActionMatch = pathname.match(/^\/api\/admin\/events\/([^/]+)\/action$/);
  if (adminEventActionMatch && req.method === 'POST') {
    if (!requireAdmin(req, res)) return;
    try {
      const eventId = adminEventActionMatch[1];
      const body = await readBody(req);
      if (body.action !== 'delete') return sendJson(res, 400, { error: 'Unsupported action.' });
      const runtime = getRuntime();
      runtime.events = runtime.events.filter((e) => e.id !== eventId);
      saveRuntime(runtime);
      return sendJson(res, 200, { ok: true });
    } catch (error) {
      return sendJson(res, 400, { error: error.message || 'Unable to delete event.' });
    }
  }
  ...
  if (pathname === '/api/deals' && req.method === 'GET') {
    const runtime = getRuntime();
    const now = new Date().toISOString();
    const deals = runtime.deals
      .filter((d) => !d.expiresAt || d.expiresAt >= now)
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    return sendJson(res, 200, { deals });
  }

  // Deals — admin POST create
  if (pathname === '/api/admin/deals' && req.method === 'POST') {
    if (!requireAdmin(req, res)) return;
    try {
      const body = await readBody(req);
      const businessName = String(body.businessName || '').trim();
      const title = String(body.title || '').trim();
      if (!businessName || !title) return sendJson(res, 400, { error: 'businessName and title are required.' });
      const runtime = getRuntime();
      const deal = {
        id: createId('DEAL'),
        businessName,
      ...
      runtime.deals.push(deal);
      saveRuntime(runtime);
      return sendJson(res, 201, { deal });
    } catch (error) {
      return sendJson(res, 400, { error: error.message || 'Unable to create deal.' });
    }
  }

  // Analytics — POST track view
  if (pathname === '/api/analytics/view' && req.method === 'POST') {
    try {
      ...
      const runtime = getRuntime();
      const listing = runtime.businessListings.find((l) => l.id === businessId);
      if (listing) { listing.viewCount = (listing.viewCount || 0) + 1; saveRuntime(runtime); }
      return sendJson(res, 200, { ok: true });
    } catch (error) {
      return sendJson(res, 400, { error: error.message || 'Unable to track view.' });
    }
  }

  // Analytics — POST track click
  if (pathname === '/api/analytics/click' && req.method === 'POST') {
    try {
      ...
      const runtime = getRuntime();
      const listing = runtime.businessListings.find((l) => l.id === businessId);
      if (listing) { listing.clickCount = (listing.clickCount || 0) + 1; saveRuntime(runtime); }
      return sendJson(res, 200, { ok: true });
    } catch (error) {
      return sendJson(res, 400, { error: error.message || 'Unable to track click.' });
    }
  }

  // Business analytics — GET for business owners
  if (pathname === '/api/business-analytics' && req.method === 'GET') {
    const auth = requireUser(req, res);
    if (!auth) return;
    const runtime = getRuntime();
    const myListings = runtime.businessListings.filter((l) => l.userId === auth.user.id);
    const analytics = myListings.map((l) => {
      const revs = runtime.reviews.filter((r) => r.businessId === l.id || r.businessId === `listing-${l.id}`);
      return {
        id: l.id,
        ...
        avgRating: revs.length ? (revs.reduce((s, r) => s + r.rating, 0) / revs.length).toFixed(1) : null
      };
    });
    return sendJson(res, 200, {
      analytics,
      totalViews: analytics.reduce((s, a) => s + a.viewCount, 0),
      totalClicks: analytics.reduce((s, a) => s + a.clickCount, 0)
    });
  }
