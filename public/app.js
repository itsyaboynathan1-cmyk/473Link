...
const categories = [
  ...
  if (els.openNowFilter) els.openNowFilter.checked = state.filters.openNow;
  if (els.minRatingFilter) els.minRatingFilter.value = String(state.filters.minRating);
...
function getFilteredServices() {
  ...
  let filtered = state.services.filter((service) => {
    ...
    if (state.filters.minRating > 0 && service.reviews > 0 && service.rating < state.filters.minRating) return false;
    ...
    .map((service) => {
      ...
      const openBadge = openStatus === true
        ...
        : openStatus === false
          ...
          : '';
      const waHref = getWhatsAppHref(service);
      const waShareHref = getWhatsAppShareHref(service);
      const tags = (service.tags || []).slice(0, 2);
      return `
        <article class="service-card card">
          <div class="service-card__media ${service.mediaClass} ${service.banner ? 'has-banner' : ''}" ${service.banner ? `style="background-image: url(\'${escapeHtml(service.banner)}\')"` : ''}>
            ...
            <button type="button" class="icon-button" data-action="favorite" data-service-id="${escapeHtml(service.id)}" aria-label="${saved ? 'Unsave' : 'Save'} service">
              ${saved ? '♥' : '♡'}
            </button>
          </div>
          <div class="service-card__body">
            ...
            <div class="service-card__price">
              ...
              <div>
                ...
                <strong>${waHref ? 'WhatsApp ready' : 'View profile'}</strong>
              </div>
            </div>
...
function renderDetails(serviceId) {
  ...
  els.detailsContent.innerHTML = `
    ...
    <div class="provider-section reviews-section" id="reviewsSection" data-business-id="${escapeHtml(service.id)}">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;margin-bottom:0.75rem;">
        <h3 style="margin:0;">Customer Reviews</h3>
        ${state.user ? `<button type="button" class="ghost-button" id="openReviewForm" style="padding:0.5rem 0.9rem;font-size:0.85rem;">Write a review</button>` : `<button type="button" class="ghost-button" data-action="open-auth" style="padding:0.5rem 0.9rem;font-size:0.85rem;">Log in to review</button>`}
      </div>
      ${state.user ? `
      <div class="review-form hidden" id="reviewFormPanel">
        <p style="margin:0 0 0.5rem;font-weight:700;font-size:0.9rem;">Your rating</p>
        ...
        <label>
          <span style="font-size:0.85rem;color:var(--muted);">Your review (optional)</span>
          <textarea id="reviewTextInput" rows="3" placeholder="Share your experience with this business…" style="margin-top:0.4rem;"></textarea>
        </label>
        <button type="button" class="primary-button" id="submitReviewBtn">Submit review</button>
      </div>` : ''}
      <div class="reviews-list" id="reviewsList"><p class="muted" style="font-size:0.9rem;">Loading reviews…</p></div>
    </div>
  `;

  // Load reviews for this business asynchronously
  if (listingId || service.source === 'starter') {
    fetchAndRenderReviews(service.id, listingId);
  }

  // Wire review form
  const openReviewFormBtn = els.detailsContent.querySelector('#openReviewForm');
  const reviewFormPanel = els.detailsContent.querySelector('#reviewFormPanel');
  if (openReviewFormBtn && reviewFormPanel) {
    openReviewFormBtn.addEventListener('click', () => {
      reviewFormPanel.classList.toggle('hidden');
      openReviewFormBtn.textContent = reviewFormPanel.classList.contains('hidden') ? 'Write a review' : 'Cancel';
    });
  }

  // Wire star rating input
  const starRow = els.detailsContent.querySelector('#starInputRow');
  const ratingInput = els.detailsContent.querySelector('#reviewRatingInput');
  if (starRow && ratingInput) {
    starRow.querySelectorAll('.star-input').forEach((star) => {
      star.addEventListener('click', () => {
        const val = Number(star.dataset.star);
        ratingInput.value = String(val);
        starRow.querySelectorAll('.star-input').forEach((s) => {
          s.classList.toggle('lit', Number(s.dataset.star) <= val);
        });
      });
    });
  }
  ...
  // Wire submit review
  const submitReviewBtn = els.detailsContent.querySelector('#submitReviewBtn');
  if (submitReviewBtn) {
    submitReviewBtn.addEventListener('click', () => submitReview(service.id, listingId));
  }
}
...
function openDetails(serviceId) {
  ...
  // Track view for real listings
  const service = findService(serviceId);
  if (service?.listingId) trackView(service.listingId);
}
...
function bindEvents() {
  ...
  els.minRatingFilter?.addEventListener('change', (event) => {
  ...
  document.querySelector('.desktop-nav')?.addEventListener('click', (event) => {
  ...
  // Tourism "Browse all businesses" button
  document.querySelector('#tourismExploreBtn')?.addEventListener('click', () => navigateTo('home'));

  // Business dashboard biz-analytics open-auth button
  document.querySelector('#bizDashboardContent')?.addEventListener('click', (event) => {
    if (event.target.closest('[data-action="open-auth"]')) openAuth('login');
  });

  els.resetFilters.addEventListener('click', () => {
    state.filters = { search: '', category: 'all', parish: 'all', sort: 'recommended', sameDay: false, openNow: false, minRating: 0 };
  ...
  els.servicesGrid.addEventListener('click', (event) => {
    ...
    const waLink = event.target.closest('.wa-btn[data-listing-id]');
    if (waLink?.dataset.listingId) trackClick(waLink.dataset.listingId);
...
// ══════════════════════════════════════════════════════════════
// NEW FEATURES: Open Now, WhatsApp Share, Reviews, Events,
// Deals, Tourism, Carousel, Business Analytics, Page Nav
// ══════════════════════════════════════════════════════════════

function isOpenNow(hours) {
  if (!hours || hours === 'Hours shared after approval') return null;
  ...
  if (daysStr.toLowerCase() === 'daily') {
    ...
    const dayMatch = daysStr.match(/^(\w+)-(\w+)$/);
    if (dayMatch) {
      const startDay = dayMap[dayMatch[1]];
      const endDay = dayMap[dayMatch[2]];
      if (startDay !== undefined && endDay !== undefined) {
        dayOpen = startDay <= endDay ? (dayNum >= startDay && dayNum <= endDay) : (dayNum >= startDay || dayNum <= endDay);
      }
    } else {
      dayOpen = daysStr.split(',').map((d) => d.trim()).some((d) => dayMap[d] === dayNum);
    }
  }
  ...
  const closeMin = parseInt(timeMatch[5], 10);
...
function getWhatsAppShareHref(service) {
  const name = String(service.name || '');
  const parish = String(service.parish || '');
  const category = String(service.category || '');
  const message = `Check out ${name} (${category}) on 473Link — Grenada's local business directory! Find them at: 473link.app`;
...
function navigateTo(page) {
  ...
  document.querySelectorAll('.mobile-nav__item').forEach((el) => el.classList.remove('active'));
  ...
  const viewMap = {
    ...
    events: 'eventsView',
    deals: 'dealsView',
    'biz-dashboard': 'bizDashboardView'
  };

  const viewId = viewMap[page] || 'homeView';
  const view = document.getElementById(viewId);
  if (view) view.classList.add('active');

  // Update active tab
  document.querySelectorAll(`[data-page="${page}"]`).forEach((el) => {
    if (el.classList.contains('page-tab') || el.classList.contains('mobile-nav__item')) {
      el.classList.add('active');
    }
  });

  // If showing events/deals, refresh them
  if (page === 'events') fetchEvents();
  if (page === 'deals') fetchDeals();
  if (page === 'biz-dashboard') renderBizDashboard();
...
async function fetchEvents() {
  try {
    ...
    const data = await response.json();
    if (!response.ok || !data.events?.length) return;
    const container = document.getElementById('eventsApiList');
    if (!container) return;
    container.innerHTML = `
      <div class="section__header" style="margin-top:1.5rem;"><div><p class="eyebrow">Live events</p><h3>Upcoming & Community Events</h3></div></div>
      <div class="events-grid">
        ${data.events.map((event) => {
          const d = new Date(event.date);
          const month = Number.isNaN(d.getTime()) ? '—' : d.toLocaleString('en', { month: 'short' }).toUpperCase();
          const day = Number.isNaN(d.getTime()) ? '—' : d.getDate();
          return `
            <div class="event-card">
              <div class="event-date-block"><strong>${escapeHtml(String(day))}</strong><span>${escapeHtml(month)}</span></div>
...
async function fetchDeals() {
  try {
    ...
    const data = await response.json();
    if (!response.ok || !data.deals?.length) return;
    const container = document.getElementById('dealsApiList');
    if (!container) return;
    container.innerHTML = `
      <div class="section__header"><div><p class="eyebrow">Exclusive</p><h3>Business Deals &amp; Promotions</h3></div></div>
    ...
    // keep default static deals
  }
}

// ── Reviews ──────────────────────────────────────────────────
function renderStars(rating, max = 5) {
  return Array.from({ length: max }, (_, i) => `<span class="star${i < Math.round(rating) ? '' : ' empty'}">★</span>`).join('');
}

async function fetchAndRenderReviews(serviceId, listingId) {
  const container = document.getElementById('reviewsList');
  if (!container) return;
  const businessId = listingId || serviceId;
  try {
    const response = await fetch(`/api/reviews?businessId=${encodeURIComponent(businessId)}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Unable to load reviews.');
    const reviews = data.reviews || [];
    if (!reviews.length) {
      container.innerHTML = `<p class="muted" style="font-size:0.9rem;">No reviews yet — be the first to share your experience!</p>`;
      return;
    }
      ...
      <div class="review-item">
        <div class="review-header">
          <div>
            ...
            <div class="star-row">${renderStars(r.rating)}</div>
          </div>
    ...
    container.innerHTML = `<p class="muted" style="font-size:0.88rem;">Unable to load reviews right now.</p>`;
  }
}

async function submitReview(serviceId, listingId) {
  if (!requireUser('Log in to leave a review.')) return;
  const businessId = listingId || serviceId;
  ...
  const original = btn?.textContent || 'Submit review';
  if (btn) { btn.disabled = true; btn.textContent = 'Submitting…'; }

  try {
    const response = await fetch('/api/reviews', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-auth-token': state.userToken },
      body: JSON.stringify({ businessId, rating, text })
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Unable to submit review.');
    document.getElementById('reviewFormPanel')?.classList.add('hidden');
    document.getElementById('openReviewForm').textContent = 'Write a review';
    if (ratingInput) ratingInput.value = '0';
    if (textInput) textInput.value = '';
    document.querySelectorAll('.star-input').forEach((s) => s.classList.remove('lit'));
    await fetchAndRenderReviews(serviceId, listingId);
...
function trackView(listingId) {
  ...
  fetch('/api/analytics/view', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ businessId: listingId })
  }).catch(() => {});
}
...
async function renderBizDashboard() {
  const container = document.getElementById('bizDashboardContent');
  if (!container) return;
  if (!state.user) {
    container.innerHTML = `
      <div class="empty-state card">
        <strong>Log in to view your business analytics.</strong>
        <p class="muted">Profile views, contact clicks, and customer reviews in one place.</p>
        <button type="button" class="primary-button" data-action="open-auth">Log in</button>
      </div>`;
    container.addEventListener('click', (e) => { if (e.target.closest('[data-action="open-auth"]')) openAuth('login'); });
    return;
  }
  container.innerHTML = `<p class="muted">Loading your business analytics…</p>`;
  try {
    const response = await fetch('/api/business-analytics', { headers: { 'x-auth-token': state.userToken } });
    const data = await response.json();
    ...
    if (!analytics.length) {
      container.innerHTML = `
        <div class="empty-state card">
          <strong>No business listings yet.</strong>
          <p class="muted">Submit your business and once it's approved, you'll see your analytics here.</p>
          <button type="button" class="primary-button" id="bizDashSubmitBtn">List your business</button>
        </div>`;
      container.querySelector('#bizDashSubmitBtn')?.addEventListener('click', () => {
    ...
    container.innerHTML = `
      <div class="biz-analytics-grid">
        <div class="analytics-metric"><span class="big-number">${data.totalViews}</span><span class="metric-name">Total profile views</span></div>
        <div class="analytics-metric"><span class="big-number">${data.totalClicks}</span><span class="metric-name">Contact clicks</span></div>
        <div class="analytics-metric"><span class="big-number">${analytics.reduce((s, a) => s + a.reviewCount, 0)}</span><span class="metric-name">Reviews received</span></div>
        <div class="analytics-metric"><span class="big-number">${analytics.filter(a => a.status === 'approved').length}</span><span class="metric-name">Live listings</span></div>
      </div>
      ${analytics.map((a) => `
        <div class="card" style="padding:1.1rem;margin-bottom:1rem;">
          <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;">
            <strong>${escapeHtml(a.businessName)}</strong>
            <span class="table-status ${a.status === 'approved' ? 'table-status--verified' : a.status === 'pending' ? 'table-status--pending' : 'table-status--rejected'}">${escapeHtml(a.status)}</span>
          </div>
          <div class="biz-analytics-grid" style="margin-top:0.75rem;grid-template-columns:repeat(2,1fr);">
            <div class="analytics-metric"><span class="big-number" style="font-size:1.5rem;">${a.viewCount}</span><span class="metric-name">Views</span></div>
            <div class="analytics-metric"><span class="big-number" style="font-size:1.5rem;">${a.clickCount}</span><span class="metric-name">Clicks</span></div>
          </div>
          ${a.reviewCount > 0 ? `<p class="muted" style="margin:0.5rem 0 0;font-size:0.88rem;">${a.reviewCount} review${a.reviewCount === 1 ? '' : 's'} · ${a.avgRating}★ average</p>` : '<p class="muted" style="margin:0.5rem 0 0;font-size:0.88rem;">No reviews yet</p>'}
        </div>`).join('')}
    `;
  } catch (error) {
    container.innerHTML = `<div class="empty-state card"><strong>Unable to load analytics.</strong><p class="muted">${escapeHtml(error.message)}</p></div>`;
  }
}
...
function buildCarousel() {
  ...
  const featured = state.services.filter((s) => s.featured || s.verified || s.rating >= 4.7).slice(0, 6);
  if (!featured.length) {
  ...
  track.innerHTML = featured.map((service) => {
    const waHref = getWhatsAppHref(service);
    return `
      <article class="service-card card" style="cursor:pointer;" data-carousel-service="${escapeHtml(service.id)}">
        <div class="service-card__media ${service.mediaClass} ${service.banner ? 'has-banner' : ''}" ${service.banner ? `style="background-image:url('${escapeHtml(service.banner)}')"` : ''}>
          ${service.logo ? `<img src="${escapeHtml(service.logo)}" class="service-logo" alt="${escapeHtml(service.name)} logo">` : ''}
          <div>
            <span class="status-pill">${escapeHtml(service.category)}</span>
  ...
  // Dots
  dotsContainer.innerHTML = featured.map((_, i) => `<button class="carousel-dot${i === 0 ? ' active' : ''}" data-idx="${i}" aria-label="Go to slide ${i + 1}"></button>`).join('');
  ...
  document.getElementById('carouselPrev')?.addEventListener('click', () => {
    ...
    updateCarouselPosition();
    resetCarouselTimer(featured.length);
  });
  ...
  // Click on carousel card opens details
  track.addEventListener('click', (e) => {
    const serviceId = e.target.closest('[data-carousel-service]')?.dataset.carouselService;
    if (serviceId && !e.target.closest('[data-action]') && !e.target.closest('.wa-btn')) {
      openDetails(serviceId);
    }
  });

  // Wire action buttons inside carousel
  track.addEventListener('click', (e) => {
  ...
  startCarouselTimer(featured.length);
}
...
async function init() {
  ...
  // Initialize events and deals in the background
  fetchEvents();
  fetchDeals();
