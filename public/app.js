...
function getInitials(name) {
  return String(name || '').split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase() || '').join('') || '4';
}
...
function renderLogoMarkup(service, large = false) {
  const src = getImageSrc(service.logo);
  const classes = large ? 'service-logo service-logo--large' : 'service-logo';
  const fallbackClass = large ? 'logo-fallback logo-fallback--large' : 'logo-fallback';
  const initials = escapeHtml(getInitials(service.name));
  if (src) {
    return `<img src="${escapeHtml(src)}" class="${classes}" alt="${escapeHtml(service.name)} logo" onerror="this.outerHTML='\x3cdiv class=&quot;${fallbackClass}&quot;\x3e${initials}\x3c/div\x3e'">`;
  }
  return `<div class="${fallbackClass}">${initials}</div>`;
}
...
function getBackgroundImage(service) {
  const custom = getImageSrc(service.coverImage);
  return custom ? `background-image:url('${custom.replace(/'/g, "%27")}');` : '';
}

function isNewBusiness(service) {
  const created = new Date(service.createdAt || 0).getTime();
  return created && (Date.now() - created) < 7 * 24 * 60 * 60 * 1000;
}

function buildBusinessSignals(service) {
  const signals = [];
  if (service.featured) signals.push('Featured');
  if (service.plan === 'founding') signals.push('Founding Partner');
  if (service.verified) signals.push('Verified by 473Link');
  if (isNewBusiness(service)) signals.push('New this week');
  if (Number(service.responseMinutes || 999) <= 30) signals.push('Responds quickly');
  return signals;
}
...
function listingToService(listing) {
  return {
    ...
    plan: String(listing.plan || 'starter'),
    ...
    tags: buildBusinessSignals({ ...listing, createdAt: listing.createdAt, responseMinutes: listing.responseMinutes })
  };
}
...
function setServices() {
  ...
  state.services = [
    ...(state.config.showStarterListings ? STARTER_SERVICES : []),
    ...approvedListings
  ];
}

function getCatalogForForms() {
  const catalog = [...STARTER_SERVICES, ...state.customListings.map((listing) => listingToService(listing))];
  return catalog.length ? catalog : STARTER_SERVICES;
}
...
function syncAuthMode() {
  ...
  els.authTitle.textContent = isSignup ? 'Create your 473Link account' : 'Log in to 473Link';
  ...
  els.authHint.textContent = isSignup
  ...
  els.authSubmitButton.textContent = isSignup ? 'Create account' : 'Log in';
  els.authModeLogin.classList.toggle('active', !isSignup);
  els.authModeSignup.classList.toggle('active', isSignup);
  els.authSignupOnly.forEach((node) => node.classList.toggle('hidden', !isSignup));
...
function renderFeaturedStrip() {
  if (!els.featuredStrip) return;
  const featured = state.services.filter((item) => item.featured).slice(0, 8);
  if (!featured.length) {
    els.featuredStrip.innerHTML = `<div class="empty-state card"><strong>No featured businesses yet.</strong><p class="muted">Feature a business in the admin dashboard and it will appear here.</p></div>`;
    return;
  }
  els.featuredStrip.innerHTML = featured.map((service) => `
    <article class="featured-card" data-action="details" data-service-id="${escapeHtml(service.id)}">
      <div class="featured-card__bg ${service.coverImage ? '' : service.mediaClass}" style="${getBackgroundImage(service)}"></div>
      <div class="featured-card__overlay"></div>
      <div class="featured-card__topbadges">
        <span class="status-pill status-pill--featured">Featured</span>
        ${service.plan === 'founding' ? '<span class="status-pill status-pill--founding">Founding Partner</span>' : ''}
      </div>
      <div class="featured-card__content">
        ${renderLogoMarkup(service)}
        <div class="rating-row"><span class="stars">${renderStars(service.rating)}</span><span>${service.rating ? `${Number(service.rating).toFixed(1)} / 5` : 'New listing'}</span></div>
        <h3>${escapeHtml(service.name)}</h3>
        <p>${escapeHtml(service.category)} · ${escapeHtml(service.parish)}</p>
      </div>
    </article>
  `).join('');
}
...
function renderServices() {
  ...
  els.servicesGrid.innerHTML = filtered.map((service) => {
    const saved = state.favorites.has(service.id);
    return `
      <article class="service-card card ${service.featured ? 'service-card--featured' : ''}">
        <div class="service-card__media ${service.mediaClass}">
          <div class="service-card__bg ${service.coverImage ? '' : service.mediaClass}" style="${getBackgroundImage(service)}"></div>
          <div class="service-card__overlay"></div>
          <div class="service-card__content">
            <div class="service-card__identity">
              <div class="service-card__topbadges">
                ${service.featured ? '<span class="status-pill status-pill--featured">Featured</span>' : ''}
                ${service.plan === 'founding' ? '<span class="status-pill status-pill--founding">Founding Partner</span>' : ''}
                ${service.verified ? '<span class="status-pill">Verified by 473Link</span>' : ''}
              </div>
              <h3>${escapeHtml(service.name)}</h3>
              <p>${escapeHtml(service.category)} · ${escapeHtml(service.parish)}</p>
              <div class="rating-row">
                <span class="stars">${renderStars(service.rating)}</span>
                <span>${service.rating ? `${Number(service.rating).toFixed(1)} / 5` : 'New listing'}</span>
                ${service.reviews ? `<span>${service.reviews} reviews</span>` : ''}
              </div>
            </div>
            <div>
              ${renderLogoMarkup(service)}
              <button type="button" class="icon-button" data-action="favorite" data-service-id="${escapeHtml(service.id)}" aria-label="Save service">${saved ? '♥' : '♡'}</button>
            </div>
          </div>
        </div>
        <div class="service-card__body">
          <p>${escapeHtml(service.summary)}</p>
          <div class="service-card__badges">${service.tags.slice(0, 4).map((item) => `<span class="badge">${escapeHtml(item)}</span>`).join('')}</div>
          <div class="service-card__price">
            <div><span class="metric-label">Starting from</span><strong>${formatStartsFrom(service.startingPrice)}</strong></div>
            <div><span class="metric-label">Hours</span><strong>${escapeHtml(service.hours || 'Hours available on profile')}</strong></div>
          </div>
...
function renderDetails(serviceId) {
  ...
  const providerLabel = service.verified ? 'Verified by 473Link' : 'Approved business';
    ...
    <div class="provider-hero ${service.mediaClass}">
      ...
      <div class="service-card__content">
        <div class="service-card__identity">
          ...
          <p>${escapeHtml(service.summary)}</p>
          <div class="rating-row"><span class="stars">${renderStars(service.rating)}</span><span>${service.rating ? `${Number(service.rating).toFixed(1)} / 5` : 'New listing'}</span>${service.reviews ? `<span>${service.reviews} reviews</span>` : ''}</div>
        </div>
        ${renderLogoMarkup(service, true)}
      </div>
    </div>
    <div class="provider-section"><h3>Business details</h3><div class="business-facts"><div><strong>Category:</strong> ${escapeHtml(service.category)}</div><div><strong>Parish:</strong> ${escapeHtml(service.parish)}</div><div><strong>Hours:</strong> ${escapeHtml(service.hours)}</div>${service.verified ? '<div><strong>Verified by 473Link:</strong> Business details confirmed by our team.</div>' : ''}</div></div>
    <div class="provider-section"><h3>About this business</h3><p>${escapeHtml(service.about)}</p></div>
    <div class="provider-section"><h3>Contact this business</h3>${renderContactActions(service)}</div>
    <div class="provider-section"><h3>Services included</h3><div class="provider-tags">${service.services.map((item) => `<span>${escapeHtml(item)}</span>`).join('')}</div></div>
    <div class="provider-section card" style="padding:1rem;"><div class="provider-cta"><button type="button" class="ghost-button" data-action="favorite" data-service-id="${escapeHtml(service.id)}">${favoriteLabel}</button><button type="button" class="primary-button" data-action="share-business" data-service-id="${escapeHtml(service.id)}">Share business</button></div></div>
  `;
}
    ...
    ? bookingRows
        .map((booking) => `
          <tr>
            ...
            <td>${formatDate(booking.date)} · ${escapeHtml(booking.time)}</td>
            <td><span class="${getStatusBadgeClass(booking.status)}">${escapeHtml(booking.status)}</span></td>
            <td>${formatCurrency(booking.amountDueNow)}</td>
          </tr>
        `)
        .join('')
          ...
          <tr>
            ...
            <td>
              <div class="table-actions">
                <button type="button" class="table-action" data-admin-action="approve_feature" data-listing-id="${escapeHtml(listing.id)}">Approve + Feature</button>
                <button type="button" class="table-action" data-admin-action="approve" data-listing-id="${escapeHtml(listing.id)}">Approve</button>
                <button type="button" class="table-action" data-admin-action="duplicate" data-listing-id="${escapeHtml(listing.id)}">Duplicate</button>
    ...
    ? rejectedRows
        .map((listing) => `
          <tr>
            ...
            <td>${formatDate(listing.reviewedAt || listing.createdAt)}</td>
            <td>
              <div class="table-actions">
...
async function fetchConfig() {
  try {
    ...
    const data = await response.json();
    if (response.ok) state.config = { ...state.config, ...data };
  } catch {
...
async function refreshCatalog() {
  ...
  if (hash.startsWith('#business=')) {
    ...
    if (findService(id)) openDetails(id);
  }
}
...
async function submitBusinessListing(event) {
  ...
  const coverFile = formData.get('coverPhotoFile');
  const uploadedLogo = logoFile && logoFile.size ? await fileToDataUrl(logoFile) : '';
  const uploadedCover = coverFile && coverFile.size ? await fileToDataUrl(coverFile) : '';
  const payload = {
    ownerName: formData.get('ownerName'),
    businessName: formData.get('businessName'),
    businessPhone: formData.get('businessPhone'),
    email: formData.get('email'),
    logo: uploadedLogo || formData.get('logo'),
    coverImage: uploadedCover || formData.get('coverImage'),
